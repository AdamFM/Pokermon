  ...
  calculate = function(self, card, context)
    if context.before and context.cardarea == G.jokers and not context.blueprint then
      ...
      for k, v in ipairs(context.scoring_hand) do
        local random_m = randomSelect(tipeKartu("ability"))
        local random_e = randomSelect(tipeKartu("edition"))
        local random_g = randomSelect(tipeKartu("seal"))
        local card_edisi = false
        if not context.scoring_hand[k].edition then 
          sendDebugMessage(context.scoring_hand[k].edition)
        else
...
local exeggutor={
  name = "exeggutor", 
  pos = {x = 11, y = 7}, 
  config = {extra = {mult_mod = 2, Xmult_multi = 1.5, suit = "Hearts", odds = 1}},
  loc_vars = function(self, info_queue, center)
    type_tooltip(self, info_queue, center)
